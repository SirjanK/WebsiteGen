...
class Model:
  def __init__(self):
    ...
    self.g_X = Generator()
    self.g_Y = Generator()
    self.optimizer_d_X = optim.Adam(self.d_X.parameters(), lr=0.0001, momentum=0.5)
    self.optimizer_d_Y = optim.Adam(self.d_Y.parameters(), lr=0.0001, momentum=0.5)
    self.optimizer_g_X = optim.Adam(self.g_X.parameters(), lr=0.0002, momentum=0.5)
    self.optimizer_g_Y = optim.Adam(self.g_Y.parameters(), lr=0.0002, momentum=0.5)
    self.counter = 1
    if USE_GPU:
        self.d_X.cuda()
        self.d_Y.cuda()
  ...
  def train(self, images, label):
    ...
    :return:
    """
    images = convert_to_cuda_var(images)
  ...
  def calculate_loss(self, images, label, images_mapped):
    ...
    if label == 0:
      ...
      cycle_loss = (self.g_Y(self.g_X(images)) - images).norm(p=1, dim=1)
      return adv_loss + LAMBDA * cycle_loss
    else:
      adv_loss = self.d_X(self.g_Y(images)).norm(dim=1)
      cycle_loss = (self.g_X(self.g_Y(images)) - images).norm(p=1, dim=1)
      return adv_loss + LAMBDA * cycle_loss
